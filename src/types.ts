...
export interface Transfer { id: string; transferCode?: string; fromId: string; toId: string; variantId: string; quantity: number; status: 'sent' | 'received' | 'cancelled'; createdAt: string; receivedAt?: string; cancelledAt?: string; cancelReason?: string }
...
export interface HppRecipe { id: string; variantId?: string; name: string; yieldQuantity: number; yieldUnit: string; materials: HppMaterial[]; additionalCosts: HppAdditionalCost[]; targetMargin: number; sellingPrice?: number; updatedAt: string }
export interface MarketplaceConfig { platform: string; adminFee: number; paymentFee: number; shippingFee: number; fixedFee: number; discount: number; updatedAt: string }
export interface PricingData { hppRecipes?: HppRecipe[]; marketplaceConfigs?: MarketplaceConfig[] }
export interface AppData { business?: BusinessProfile; users: User[]; locations: Location[]; products: Product[]; balances: Balance[]; transfers: Transfer[]; sales: Sale[]; movements: Movement[]; stockCounts: StockCount[]; suppliers?: Supplier[]; receipts?: StockReceipt[]; returns?: StockReturn[]; employees?: Employee[]; attendanceSettings?: AttendanceSetting[]; attendances?: Attendance[]; loans?: Loan[]; payrolls?: Payroll[]; pricing?: PricingData }
