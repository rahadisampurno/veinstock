...
export interface Variant { id: string; name: string; sku: string; cost: number; price: number; resellerPrice: number; minStock: number; minStockByLocation?: Record<string, number>; active?: boolean; imageUrl?: string }
...
export interface StockReceipt { id: string; sourceType: 'supplier' | 'production'; supplierId?: string; supplierName?: string; locationId: string; variantId: string; quantity: number; unitCost: number; note?: string; createdAt: string; status: 'completed' | 'cancelled'; cancelledAt?: string; cancelReason?: string; createdBy?: string; }
