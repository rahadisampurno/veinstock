...
export interface Variant { id: string; name: string; sku: string; barcode?: string; packageWeight?: string; flavor?: string; spiceLevel?: string; cost: number; price: number; resellerPrice: number; minStock: number; minStockByLocation?: Record<string, number>; active?: boolean; imageUrl?: string }
...
export interface Sale { id: string; locationId: string; channel: Channel; customer?: string; note?: string; total: number; payment: string; createdAt: string; items: SaleItem[]; cashierId?: string; status?: 'pending_print' | 'completed' | 'voided'; printedAt?: string; printedBy?: string; cancelledAt?: string; cancelReason?: string }
...
export interface Employee { id: string; userId: string; locationId?: string; position: string; monthlySalary: number; active: boolean; joinDate?: string }
...
export interface Payroll { id: string; employeeId: string; period: string; grossAmount: number; status: 'paid'; paidAt: string; note?: string; proofUrl?: string; employeeName?: string; positionSnapshot?: string; locationNameSnapshot?: string }
