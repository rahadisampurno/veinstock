...
import { calculateHppCosts, materialLineCost, nonNegativeNumber } from "./utils/hpp";
import type {
  AppData,
  Channel,
  HppAdditionalCost,
...
function HppMarketplaceCalculator({ data, runCommand, notify }: { data: AppData; runCommand: (path: string, payload: object, method?: "POST" | "PATCH") => Promise<AppData>; notify: (message: string) => void }) {
  ...
  const numberInput = (label: string, value: number, onChange: (value: number) => void, suffix = "Rp", step = "any") => <label className="field"><span>{label}</span><div className="money-input">{suffix && <small>{suffix}</small>}<input type="number" min="0" step={step} value={Number.isFinite(value) ? value : 0} onInput={event => onChange(nonNegativeNumber(event.currentTarget.value))} onBlur={event => onChange(nonNegativeNumber(event.currentTarget.value))} /></div></label>;
  return <PageBlock title="HPP & Marketplace" desc="Hitung HPP produksi serta simulasi biaya marketplace dari satu menu yang terhubung ke produk Menengs.">
    ...
    {workspace === "hpp" ? <div className="hpp-wizard">
      ...
      {hppStep === 1 && <section className="card hpp-step-card">
        ...
        <div className="pricing-table-wrap"><table className="pricing-table hpp-material-table"><thead><tr><th>Nama bahan baku</th><th>Jumlah dipakai</th><th>Satuan dipakai</th><th>Harga satuan</th><th>Harga per</th><th>Total biaya</th><th>Aksi</th></tr></thead><tbody>{recipe.materials.map(item => <tr key={item.id}><td><input aria-label="Nama bahan baku" value={item.name} onChange={event => updateMaterial(item.id, { name: event.target.value })} placeholder="Contoh: Makaroni" /></td><td><input aria-label="Jumlah bahan" type="number" min="0" step="any" inputMode="decimal" value={item.quantity} onInput={event => updateMaterial(item.id, { quantity: nonNegativeNumber(event.currentTarget.value) })} onBlur={event => updateMaterial(item.id, { quantity: nonNegativeNumber(event.currentTarget.value) })} /></td><td><select aria-label="Satuan bahan" value={item.unit} onChange={event => updateMaterial(item.id, { unit: event.target.value })}><option>Pcs</option><option>Gram</option><option>Kg</option><option>Liter</option><option>Pack</option><option>Box</option></select></td><td><input aria-label="Harga satuan bahan" type="number" min="0" step="any" inputMode="decimal" value={item.unitCost} onInput={event => updateMaterial(item.id, { unitCost: nonNegativeNumber(event.currentTarget.value) })} onBlur={event => updateMaterial(item.id, { unitCost: nonNegativeNumber(event.currentTarget.value) })} /></td><td><span>{item.unit}</span></td><td><b>{money(materialLineCost(item))}</b></td><td><button className="icon-btn danger-icon" aria-label="Hapus bahan" disabled={recipe.materials.length === 1} onClick={() => setRecipe(current => ({ ...current, materials: current.materials.filter(material => material.id !== item.id) }))}><Trash2 /></button></td></tr>)}</tbody></table></div>
