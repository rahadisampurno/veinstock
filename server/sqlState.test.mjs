...
import { syncStateToSQL } from './sqlState.mjs';
...
describe('SQL state synchronization', () => {
  ...
  it('rewrites sale items idempotently and removes duplicate line items', async () => {
    ...
    const item = { variantId: 'variant-1', quantity: 2, price: 10000, discount: 0, subtotal: 20000 };
    await syncStateToSQL(conn, 'org-1', {
    ...
    expect(insertedItems[0].params).toEqual(['sale-1', 'variant-1', 2, 10000, 0, 20000]);
  });
});
